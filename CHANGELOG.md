...
- **i18n: tipos de acción** (ítem #3): los 34 tipos del selector (label + descripción) traducidos ES/EN. `actionData.ts` ahora guarda **claves** (`act.<tipo>.label/.desc`) en vez de texto — el diccionario i18n es la fuente única (sin duplicación). Cubre el picker principal, la lista de secuencia y los selectores de toggle-off/branch/acción-extra. (Campos de config por acción y presets → ítems #4-#5.)
