...
const SERIES = [
  {
    l: 'CPU',
    c: '#ff3d3d',
    data: [40,35,42,45,60,55,50,68,72,65,58,50,45,40,42,48,52,58,55,50,45,40,38,42,45,50,55,60,58,55,50,48],
  },
  {
    l: 'GPU',
    c: '#a855f7',
    data: [12,14,18,22,20,18,15,12,10,8,6,8,10,15,20,25,30,35,30,25,20,15,12,10,8,8,10,12,15,18,20,18],
  },
  {
    l: 'RAM',
    c: '#00d4ff',
    data: Array.from({ length: 32 }, (_, i) => 38 + Math.sin(i * 0.3) * 4 + i * 0.1),
  },
  {
    l: 'NET',
    c: '#ffb800',
    data: [5,8,12,20,18,15,10,8,30,45,35,25,20,15,12,10,8,6,5,8,10,15,22,28,35,40,38,32,25,20,15,12],
  },
];
...
export function MonitorB({ onClose }: MonitorBProps) {
  return (
    <div
      style={{
        width: '100%',
    ...
    >
      ...
      >
        <DotLabel size={10} color="#fff" spacing={2}>
          OSCILLOSCOPE
        </DotLabel>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
          ...
          >
            {SERIES.map((s) => {
              const path = s.data
                .map(
                  (v, i) =>
                    `${i === 0 ? 'M' : 'L'} ${(i / (s.data.length - 1)) * 320} ${220 - (v / 100) * 220}`
                )
                .join(' ');
              return (
                <path key={s.l} d={path} stroke={s.c} strokeWidth="1.5" fill="none" opacity={0.9} />
              );
            })}
          </svg>
          ...
          >
            {SERIES.map((s) => (
              <div key={s.l} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontFamily: VD.mono, fontSize: 9, color: s.c, letterSpacing: 1 }}>
                  {s.l}
                </span>
                <DotText
                  text={String(Math.round(s.data[s.data.length - 1])).padStart(2, '0')}
                  dotSize={3}
                  gap={1}
                  color="#fff"
                />
        ...
        {/* Process list */}
        <div style={{ width: 280, padding: 14, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
              ...
              >
                ...
                <div
                  style={{
                    ...
                    bottom: 0,
                    height: 1,
                    width: `${Math.min(100, parseFloat(p.cpu) * 10)}%`,
                    background: '#ff3d3d',
                    opacity: 0.4,
                  }}
                />
