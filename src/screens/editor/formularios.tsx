...
import {
  Field, Btn, FolderButtonSlot, BranchActionRow, estiloEntrada,
} from './comunes';
import type { AudioDevice, ButtonAction, FolderButton, RGBDeviceInfo, RGBProfile } from '../../types';

/**
 * Un formulario por tipo de accion, en un mapa.
 *
 * Antes era una cadena de `{action.type === 'x' && (...)}` de treinta ramas
 * dentro del paso de configuracion: complejidad ciclomatica 108, y para tocar
 * un solo tipo habia que moverse por las otras veintinueve.
 *
 * Todos reciben el mismo bag de props aunque cada uno use cuatro o cinco. Es a
 * proposito: el paso no tiene que saber que necesita cada formulario, y añadir
 * un tipo nuevo no obliga a tocar quien los dibuja.
 */
...
function FormApp(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
...
function FormWeb(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormShortcut(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormScript(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormAudioDevice(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormHotkey(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormClipboard(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormTypeText(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormKillProcess(p: PropsFormulario) {
  const VD = useTheme();
  const t = useT();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormVolumeSet(p: PropsFormulario) {
  const VD = useTheme();
  const t = useT();
  const tf = useFieldText();
  const { accent, action, setAction } = p;
...
function FormBrightness(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const { accent, action, setAction } = p;
  return (
...
function FormNotify(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormSetVar(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormIncrVar(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormWebhook(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormTts(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormRgbColor(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormRgbMode(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction, rgbConnected, rgbDevices } = p;
...
function FormRgbProfile(p: PropsFormulario) {
  const VD = useTheme();
  const t = useT();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormFolder(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
...
function FormWindowSnap(p: PropsFormulario) {
  const VD = useTheme();
  const t = useT();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
...
function FormBranch(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { accent, action, setAction } = p;
...
function FormCountdown(p: PropsFormulario) {
  const VD = useTheme();
  const tf = useFieldText();
  const inputStyle = estiloEntrada(VD);
  const { action, setAction } = p;
...
function FormMacro(p: PropsFormulario) {
  const tf = useFieldText();
  const { accent, action, setAction } = p;
  return (
    <>
